...
public class PlayChess extends Activity {
					...
					public void onClick(View v) {
						if (selected != null) {
							if (squares[x][y] == selected) {
								squares[x][y].setBackgroundColor(squares[x][y]
								        .isWhite() ? Color.WHITE : Color.GRAY);
								selected = null;
								return;
							} else {
								...
								try {
									...
									ArrayList<Location> locs = e.getPiece()
									        .getMoves();
									if (locs.isEmpty())
										sb.append("This piece has no available moves.");
									else {
										sb.append(e.getPiece()
										        + " can move to: ");
										for (Location loc : e.getPiece()
										        .getMoves()) {
											sb.append(loc);
											sb.append(' ');
										}
	...
	private void setupBoard() {
		for (int x = 7; x >= 0; x--) {
			for (int y = 0; y < 8; y++) {
				...
				squares[r][c] = new Square(this, (((r + c) % 2) != 0) ? true
				        : false, location);
	...
	private void switchImage(Square im) {
		...
		Globals.getInstance().toaster(getApplicationContext(),
		        "location:" + loc);
		if (game.getGrid().isOccupied(loc)) {
			...
			Globals.getInstance().toaster(getApplicationContext(),
			        "type: " + type);
			if (type.equals("")) {
				im.setImageResource(p.isWhite() ? R.drawable.whitepawn
				        : R.drawable.blackpawn);
			} else if (type.equals("N")) {
				im.setImageResource(p.isWhite() ? R.drawable.whiteknight
				        : R.drawable.blackknight);
			} else if (type.equals("B")) {
				im.setImageResource(p.isWhite() ? R.drawable.whitebishop
				        : R.drawable.blackbishop);
			} else if (type.equals("R")) {
				im.setImageResource(p.isWhite() ? R.drawable.whiterook
				        : R.drawable.blackrook);
			} else if (type.equals("Q")) {
				im.setImageResource(p.isWhite() ? R.drawable.whitequeen
				        : R.drawable.blackqueen);
			} else if (type.equals("K")) {
				im.setImageResource(p.isWhite() ? R.drawable.whiteking
				        : R.drawable.blackking);
			}
		} else {
			im.setImageResource(R.drawable.transparent);
	...
	/*
	 * private void renderBoard() { // linearLayout = (LinearLayout)
	 * this.findViewById(R.id.chessboard); linearLayout.removeView(table);
	 * linearLayout.removeAllViews(); linearLayout.invalidate();
	 * linearLayout.refreshDrawableState(); TableLayout newTable = new
	 * TableLayout(this); newTable.removeAllViews();
	 * linearLayout.addView(newTable); newTable.setLayoutParams(new
	 * TableLayout.LayoutParams(LayoutParams.FILL_PARENT,
	 * LayoutParams.WRAP_CONTENT)); newTable.setStretchAllColumns(true);
	 * newTable.setOrientation(LinearLayout.VERTICAL); for(int rank = 7; rank >=
	 * 0; rank--) { tr.removeAllViews(); TableRow newTR = new TableRow(this);
	 * newTable.addView(newTR); newTR.setLayoutParams(new
	 * TableLayout.LayoutParams( LayoutParams.WRAP_CONTENT,
	 * LayoutParams.WRAP_CONTENT)); Square[][] temp = new Square[8][8]; for(int
	 * file = 0; file < 8; file++) { Location loc = new Location(rank, file);
	 * temp[rank][file] = squares[rank][file];
	 * if(game.getGrid().isOccupied(loc)) { Piece p = game.getGrid().get(loc);
	 * String type = p.getAlgebraicName(); if(type.equals("")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whitepawn :
	 * R.drawable.blackpawn); } else if(type.equals("N")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whiteknight :
	 * R.drawable.blackknight); } else if(type.equals("B")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whitebishop :
	 * R.drawable.blackbishop); } else if(type.equals("R")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whiterook :
	 * R.drawable.blackrook); } else if(type.equals("Q")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whitequeen :
	 * R.drawable.blackqueen); } else if(type.equals("K")) {
	 * temp[rank][file].setImageResource(p.isWhite() ? R.drawable.whiteking :
	 * R.drawable.blackking); } } else {
	 ...
	 * temp[rank][file].setMaxWidth(20); if (squares[rank][file].isWhite()){
	 * temp[rank][file].setBackgroundColor(Color.WHITE); } else {
	 * temp[rank][file].setBackgroundColor(Color.GRAY); }
	 * temp[rank][file].setLayoutParams(new TableRow.LayoutParams(
	 * TableRow.LayoutParams.WRAP_CONTENT, TableRow.LayoutParams.WRAP_CONTENT));
	 * // newTR.removeAllViews(); // newTR.invalidate(); //
	 * newTR.refreshDrawableState(); View parent = ((View)
	 * temp[rank][file].getParent()); newTR.removeView(temp[rank][file]);
	 * newTR.addView(temp[rank][file]); //** end here *** } } } //
	 */
	...
	public void ai() {
		try {
			...
			Toast.makeText(this, "The computer has no moves for you.",
			        Toast.LENGTH_SHORT).show();
		}
	}
...
//				                startActivity(intent);
	...
	private void saveGame() {
		...
		alert.setMessage("Enter a title for this saved game, or hit 'Cancel' to abort.");
		...
		alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				String str = new String(charseq);
				Globals.getSavedGames().add(game.toPlayback(str));
			}
		});
	...
	public void undo() {
		if (game.canUndo()) {
			...
			Toast.makeText(
			        this,
			        "Last move taken back. It's "
			                + (game.isWhitesTurn() ? "white's" : "black's")
			                + " turn.", Toast.LENGTH_SHORT).show();
		} else
			Toast.makeText(this, "You cannot take back any more moves.",
			        Toast.LENGTH_SHORT).show();
		renderBoard();
	}
	...
	public void onBackPressed() {
		new AlertDialog.Builder(this)
		        ...
		        .setMessage(
		                "Are you sure you want to quit this game? The game will not be saved.")
		        .setPositiveButton("Yes",
		                new DialogInterface.OnClickListener() {
			                @Override
			                public void onClick(DialogInterface dialog,
